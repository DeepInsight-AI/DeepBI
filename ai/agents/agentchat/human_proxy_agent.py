...
class HumanProxyAgent(Agent):
    ...
    def __init__(
        ...
        # self.register_reply([Agent, None], HumanProxyAgent.generate_oai_reply)
        self.register_reply([Agent, None], HumanProxyAgent.check_human_reply)
        self.register_reply([Agent, None], HumanProxyAgent.generate_function_call_reply)
        # self.register_reply([Agent, None], HumanProxyAgent.check_termination_and_human_reply)
        self.register_reply([Agent, None], HumanProxyAgent.generate_code_execution_reply)
    ...
    async def tell_answer(self, answer_str):
        try:
            ...
            receive_json_str = await websocket.recv()
            print(str(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())) + ' ---- ' + "got a message:{}".format(
                receive_json_str))
            logger.info(
                "from user:[{}".format(
                    self.user_name) + "] , " + self.name + " got a message:{}".format(
                    receive_json_str))

            receive_json = json.loads(receive_json_str)
            r_state = receive_json['state']

            if receive_json['sender'] == 'user' and receive_json['data'][
                'data_type'] == 'question':
    ...
    async def ask_user(self, q_str):
        try:
            ...
            q_str = str(q_str).replace("TERMINATE", "")
    ...
    async def check_human_reply(
        ...
        # print('messages: ', messages)
        context = str(context).replace("TERMINATE", "")

        response = await self.ask_user(context)
