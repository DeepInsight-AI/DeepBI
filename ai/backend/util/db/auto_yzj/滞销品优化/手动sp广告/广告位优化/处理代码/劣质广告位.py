...
def main(path, brand, cur_time, country):
    ...
    file_name = "手动_劣质广告位" + '_' + brand + '_' + country + '_' + cur_time + '.csv'
    ...
    for index, row in result_data.iterrows():
        api.create_campaign_placement_info(country, brand, '滞销品优化', '手动_劣质', row['campaignName'],
                                           row['campaignId'], row['placementClassification'], row['bid'],
                                           row['new_bid'], row['ACOS_7d'],row['total_clicks_7d'], row['total_sales14d_7d'],
