...
def main(path, brand, cur_time, country):
    ...
    api2 = DbSpTools(brand,country)
    for index, row in result.iterrows():
        new_campaignId,new_campaignName,new_adGroupId = api2.select_sp_campaignid_search_term(country,cur_time,row['campaignId'])
    ...
    for index, row in output.iterrows():
        api.create_search_term_info(country, brand, '滞销品优化', '自动_优质', row['campaignName'],row['campaignId'],
