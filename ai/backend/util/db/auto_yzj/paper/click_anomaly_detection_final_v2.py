# filename: click_anomaly_detection_final_v2.py
...
# 计算近7天和近30天的日均点击量
# 将分母用合理值替代处理零值情况
data['avg_clicks_7d'] = data['total_clicks_7d'] / data['total_clicks_7d'].replace(0, 1).apply(lambda x: 7 if x > 6 else 1)
data['avg_clicks_30d'] = data['total_clicks_30d'] / data['total_clicks_30d'].replace(0, 1).apply(lambda x: 30 if x > 29 else 1)
