...
def main(path, brand, cur_time, country, version: int = 2):
    ...
    if version == 1:
        ...
        for index, row in df.iterrows():
            ...
            if 0.27 < row['ACOS_7d'] <= 0.5 and 0 < row['ACOS_30d'] <= 0.5 and row['ORDER_1m'] < 5 and row['ACOS_3d'] >= 0.27:
                new_keywordBid = max(0.05, row['keywordBid'] / ((row['ACOS_7d'] - 0.27) / 0.27 + 1)) if row['keywordBid'] >= 0.05 else row['keywordBid']
                bid_adjust = 0
                action_reason = "定义一：更新出价"

            elif row['ACOS_7d'] > 0.5 and row['ACOS_30d'] <= 0.36 and row['ACOS_3d'] > 0.27:
                new_keywordBid = max(0.05, row['keywordBid'] / ((row['ACOS_7d'] - 0.27) / 0.27 + 1)) if row['keywordBid'] >= 0.05 else row['keywordBid']
                bid_adjust = 0
                action_reason = "定义二：更新出价"
            ...
            elif 0.27 < row['ACOS_7d'] <= 0.5 and 0 < row['ACOS_30d'] <= 0.5 and row['ORDER_1m'] < 5 and row['total_sales14d_3d'] == 0:
                new_keywordBid = max(0.05, row['keywordBid'] / ((row['ACOS_7d'] - 0.27) / 0.27 + 1)) if row['keywordBid'] >= 0.05 else row['keywordBid']
                bid_adjust = 0
            ...
            elif 0.27 < row['ACOS_7d'] <= 0.5 and row['ACOS_30d'] > 0.5 and row['ORDER_1m'] < 5 and row['ACOS_3d'] >= 0.27:
                new_keywordBid = max(0.05, row['keywordBid'] / ((row['ACOS_7d'] - 0.27) / 0.27 + 1)) if row['keywordBid'] >= 0.05 else row['keywordBid']
                bid_adjust = 0
                action_reason = "定义十：更新出价"

            elif 0.27 < row['ACOS_7d'] <= 0.5 and row['ACOS_30d'] > 0.5 and row['total_sales14d_3d'] == 0:
    ...
    for index, row in output_df.iterrows():
        api.create_keyword_info(country, brand, '滞销品优化', '手动_劣质', row['keyword'], row['keywordId'],
                                ...
                                row['new_keywordBid'], row['ACOS_30d'], row['ORDER_1m'], row['total_clicks_30d'], row['total_sales14d_30d'], row['total_cost_30d'],None,
                                row['ACOS_7d'], row['total_clicks_7d'], row['total_sales14d_7d'], row['total_cost_7d'], row['ACOS_3d'], row['total_sales14d_3d'],
                                row['total_cost_3d'], row['action_reason'], row['bid_adjust'], cur_time,
                                datetime.now(), 0)
