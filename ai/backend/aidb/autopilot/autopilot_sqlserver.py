...
class AutopilotSqlserver(Autopilot):

    async def deal_question(self, json_str, message):
        """
        Process sqlserver data source and select the corresponding workflow
        """
        ...
        if q_sender == 'user':
            ...
            if q_data_type == CONFIG.type_comment:
                ...
                if CONFIG.database_model == 'online':
                    ...
                    if if_suss:
                        self.agent_instance_util.base_sqlserver_info = ' When connecting to the database, be sure to bring the port. This is sqlserver database info :' + '\n' + str(
                            db_info)
                ...
                await self.put_message(200, receiver=CONFIG.talker_bi, data_type=CONFIG.type_comment_second,
                                       content='')
            elif q_data_type == 'sqlserver_code' or q_data_type == 'chart_code' or q_data_type == 'delete_chart' or q_data_type == 'ask_data':
    ...
    async def task_base(self, qustion_message):
        """ Task type: sqlserver data analysis"""
        try:
            error_times = 0
            for i in range(max_retry_times):
                try:
                    base_sqlserver_assistant = self.get_agent_base_sqlserver_assistant()
                    python_executor = self.agent_instance_util.get_agent_python_executor()

                    await python_executor.initiate_chat(
                        base_sqlserver_assistant,
                        message=self.agent_instance_util.base_message + '\n' + self.question_ask + '\n' + str(
                            qustion_message),
                    )

                    answer_message = python_executor.chat_messages[base_sqlserver_assistant]
    ...
    def get_agent_base_sqlserver_assistant(self):
        """ Basic Agent, processing sqlserver data source """
        base_sqlserver_assistant = AssistantAgent(
            name="base_sqlserver_assistant",
            system_message="""You are a helpful AI assistant.
                  ...
                  Be careful to avoid using sqlserver special keywords in sqlserver code.
                  """ + '\n' + self.agent_instance_util.base_sqlserver_info + '\n' + CONFIG.python_base_dependency + '\n' + self.agent_instance_util.quesion_answer_language,
        ...
        return base_sqlserver_assistant
    ...
    async def task_generate_echart(self, qustion_message):
        try:
            ...
            for i in range(max_retry_times):
                try:
                    sqlserver_echart_assistant = self.agent_instance_util.get_agent_sqlserver_echart_assistant(
                        use_cache=use_cache)
                    python_executor = self.agent_instance_util.get_agent_python_executor()

                    await python_executor.initiate_chat(
                        sqlserver_echart_assistant,
                        message=self.agent_instance_util.base_message + '\n' + self.question_ask + '\n' + str(
                            qustion_message),
                    )

                    answer_message = sqlserver_echart_assistant.chat_messages[python_executor]
