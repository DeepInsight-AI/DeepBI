...
if ! command -v psql &> /dev/null; then
    # shellcheck disable=SC2162
    read -p "自动安装 postgresql12 ？(Y/N): " confirm
    if [[ $confirm == "Y" || $confirm == "y" ]]; then
...
echo "设置用户连接"
sudo sh -c "sed -i '/^#\s*TYPE/ahost deepbi deepbi 127.0.0.1/32  md5' /etc/postgresql/12/main/pg_hba.conf && service postgresql restart "
