...
_REDIS_URL = os.environ.get(
    "DEEPBI_REDIS_URL", os.environ.get("DEEPBI_REDIS_URL", "redis://redis:6379/0")
)
# This is the one to use for Bi' own connection:
REDIS_URL = add_decode_responses_to_redis_url(_REDIS_URL)
PROXIES_COUNT = int(os.environ.get("DEEPBI_PROXIES_COUNT", "1"))
...
STATSD_PREFIX = os.environ.get("DEEPBI_STATSD_PREFIX", "deepbi")
STATSD_USE_TAGS = parse_boolean(os.environ.get("DEEPBI_STATSD_USE_TAGS", "false"))
...
QUERY_RESULTS_CLEANUP_ENABLED = parse_boolean(
    os.environ.get("DEEPBI_QUERY_RESULTS_CLEANUP_ENABLED", "true")
)
QUERY_RESULTS_CLEANUP_COUNT = int(
    os.environ.get("DEEPBI_QUERY_RESULTS_CLEANUP_COUNT", "100")
)
QUERY_RESULTS_CLEANUP_MAX_AGE = int(
    os.environ.get("DEEPBI_QUERY_RESULTS_CLEANUP_MAX_AGE", "7")
)

SCHEMAS_REFRESH_SCHEDULE = int(os.environ.get("DEEPBI_SCHEMAS_REFRESH_SCHEDULE", 30))

AUTH_TYPE = os.environ.get("DEEPBI_AUTH_TYPE", "api_key")
INVITATION_TOKEN_MAX_AGE = int(
    os.environ.get("DEEPBI_INVITATION_TOKEN_MAX_AGE", 60 * 60 * 24 * 7)
)

# The secret key to use in the Flask app for various cryptographic features
SECRET_KEY = os.environ.get("DEEPBI_COOKIE_SECRET", "c292b0a3ca32397cdb050e233733900f")
...
DATASOURCE_SECRET_KEY = os.environ.get("DEEPBI_SECRET_KEY", SECRET_KEY)

# Whether and how to redirect non-HTTP requests to HTTPS. Disabled by default.
ENFORCE_HTTPS = parse_boolean(os.environ.get("DEEPBI_ENFORCE_HTTPS", "false"))
ENFORCE_HTTPS_PERMANENT = parse_boolean(
    os.environ.get("DEEPBI_ENFORCE_HTTPS_PERMANENT", "false")
)
# Whether file downloads are enforced or not.
ENFORCE_FILE_SAVE = parse_boolean(os.environ.get("DEEPBI_ENFORCE_FILE_SAVE", "true"))

# Whether api calls using the json query runner will block private addresses
ENFORCE_PRIVATE_ADDRESS_BLOCK = parse_boolean(
    os.environ.get("DEEPBI_ENFORCE_PRIVATE_IP_BLOCK", "true")
)

# Whether to use secure cookies by default.
COOKIES_SECURE = parse_boolean(
    os.environ.get("DEEPBI_COOKIES_SECURE", str(ENFORCE_HTTPS))
)
# Whether the session cookie is set to secure.
SESSION_COOKIE_SECURE = parse_boolean(
    os.environ.get("DEEPBI_SESSION_COOKIE_SECURE") or str(COOKIES_SECURE)
)
# Whether the session cookie is set HttpOnly.
SESSION_COOKIE_HTTPONLY = parse_boolean(
    os.environ.get("DEEPBI_SESSION_COOKIE_HTTPONLY", "true")
)
SESSION_EXPIRY_TIME = int(os.environ.get("DEEPBI_SESSION_EXPIRY_TIME", 60 * 60 * 6))

# Whether the session cookie is set to secure.
REMEMBER_COOKIE_SECURE = parse_boolean(
    os.environ.get("DEEPBI_REMEMBER_COOKIE_SECURE") or str(COOKIES_SECURE)
)
# Whether the remember cookie is set HttpOnly.
REMEMBER_COOKIE_HTTPONLY = parse_boolean(
    os.environ.get("DEEPBI_REMEMBER_COOKIE_HTTPONLY", "true")
)
# The amount of time before the remember cookie expires.
REMEMBER_COOKIE_DURATION = int(
    os.environ.get("DEEPBI_REMEMBER_COOKIE_DURATION", 60 * 60 * 24 * 31)
)
...
FRAME_OPTIONS = os.environ.get("DEEPBI_FRAME_OPTIONS", "deny")
FRAME_OPTIONS_ALLOW_FROM = os.environ.get("DEEPBI_FRAME_OPTIONS_ALLOW_FROM", "")

# Whether and how to send Strict-Transport-Security response headers.
# See https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Strict-Transport-Security
# for more information.
HSTS_ENABLED = parse_boolean(
    os.environ.get("DEEPBI_HSTS_ENABLED") or str(ENFORCE_HTTPS)
)
HSTS_PRELOAD = parse_boolean(os.environ.get("DEEPBI_HSTS_PRELOAD", "false"))
HSTS_MAX_AGE = int(os.environ.get("DEEPBI_HSTS_MAX_AGE", talisman.ONE_YEAR_IN_SECS))
HSTS_INCLUDE_SUBDOMAINS = parse_boolean(
    os.environ.get("DEEPBI_HSTS_INCLUDE_SUBDOMAINS", "false")
)
...
CONTENT_SECURITY_POLICY = os.environ.get(
    "DEEPBI_CONTENT_SECURITY_POLICY",
...
CONTENT_SECURITY_POLICY_REPORT_URI = os.environ.get(
    "DEEPBI_CONTENT_SECURITY_POLICY_REPORT_URI", ""
)
CONTENT_SECURITY_POLICY_REPORT_ONLY = parse_boolean(
    os.environ.get("DEEPBI_CONTENT_SECURITY_POLICY_REPORT_ONLY", "false")
)
CONTENT_SECURITY_POLICY_NONCE_IN = array_from_string(
    os.environ.get("DEEPBI_CONTENT_SECURITY_POLICY_NONCE_IN", "")
)
...
REFERRER_POLICY = os.environ.get(
    "DEEPBI_REFERRER_POLICY", "strict-origin-when-cross-origin"
)
...
SAML_SCHEME_OVERRIDE = os.environ.get("DEEPBI_SAML_SCHEME_OVERRIDE", "")

SAML_ENCRYPTION_PEM_PATH = os.environ.get("DEEPBI_SAML_ENCRYPTION_PEM_PATH", "")
SAML_ENCRYPTION_CERT_PATH = os.environ.get("DEEPBI_SAML_ENCRYPTION_CERT_PATH", "")
...
# this by setting DEEPBI_REMOTE_USER_HEADER.
#
...
REMOTE_USER_LOGIN_ENABLED = parse_boolean(
    os.environ.get("DEEPBI_REMOTE_USER_LOGIN_ENABLED", "false")
)
REMOTE_USER_HEADER = os.environ.get(
    "DEEPBI_REMOTE_USER_HEADER", "X-Forwarded-Remote-User"
)
...
LDAP_LOGIN_ENABLED = parse_boolean(os.environ.get("DEEPBI_LDAP_LOGIN_ENABLED", "false"))
# Bind LDAP using SSL. Default is False
LDAP_SSL = parse_boolean(os.environ.get("DEEPBI_LDAP_USE_SSL", "false"))
# Choose authentication method(SIMPLE, ANONYMOUS or NTLM). Default is SIMPLE
LDAP_AUTH_METHOD = os.environ.get("DEEPBI_LDAP_AUTH_METHOD", "SIMPLE")
# The LDAP directory address (ex. ldap://10.0.10.1:389)
LDAP_HOST_URL = os.environ.get("DEEPBI_LDAP_URL", None)
...
LDAP_BIND_DN = os.environ.get("DEEPBI_LDAP_BIND_DN", None)
LDAP_BIND_DN_PASSWORD = os.environ.get("DEEPBI_LDAP_BIND_DN_PASSWORD", "")
# AD/LDAP email and display name keys
LDAP_DISPLAY_NAME_KEY = os.environ.get("DEEPBI_LDAP_DISPLAY_NAME_KEY", "displayName")
LDAP_EMAIL_KEY = os.environ.get("DEEPBI_LDAP_EMAIL_KEY", "mail")
# Prompt that should be shown above username/email field.
LDAP_CUSTOM_USERNAME_PROMPT = os.environ.get(
    "DEEPBI_LDAP_CUSTOM_USERNAME_PROMPT", "LDAP/AD/SSO username:"
)
# LDAP Search DN TEMPLATE (for AD this should be "(sAMAccountName=%(username)s)"")
LDAP_SEARCH_TEMPLATE = os.environ.get(
    "DEEPBI_LDAP_SEARCH_TEMPLATE", "(cn=%(username)s)"
)
# The schema to bind to (ex. cn=users,dc=ORG,dc=local)
LDAP_SEARCH_DN = os.environ.get(
    "DEEPBI_LDAP_SEARCH_DN", os.environ.get("DEEPBI_SEARCH_DN")
)

STATIC_ASSETS_PATH = fix_assets_path(
    os.environ.get("DEEPBI_STATIC_ASSETS_PATH", "../client/dist/")
)
FLASK_TEMPLATE_PATH = fix_assets_path(
    os.environ.get("DEEPBI_FLASK_TEMPLATE_PATH", STATIC_ASSETS_PATH)
)
# Time limit (in seconds) for scheduled queries. Set this to -1 to execute without a time limit.
SCHEDULED_QUERY_TIME_LIMIT = int(
    os.environ.get("DEEPBI_SCHEDULED_QUERY_TIME_LIMIT", -1)
)

# Time limit (in seconds) for adhoc queries. Set this to -1 to execute without a time limit.
ADHOC_QUERY_TIME_LIMIT = int(os.environ.get("DEEPBI_ADHOC_QUERY_TIME_LIMIT", -1))

JOB_EXPIRY_TIME = int(os.environ.get("DEEPBI_JOB_EXPIRY_TIME", 3600 * 12))
JOB_DEFAULT_FAILURE_TTL = int(
    os.environ.get("DEEPBI_JOB_DEFAULT_FAILURE_TTL", 7 * 24 * 60 * 60)
)

LOG_LEVEL = os.environ.get("DEEPBI_LOG_LEVEL", "INFO")
LOG_STDOUT = parse_boolean(os.environ.get("DEEPBI_LOG_STDOUT", "false"))
LOG_PREFIX = os.environ.get("DEEPBI_LOG_PREFIX", "")
LOG_FORMAT = os.environ.get(
    "DEEPBI_LOG_FORMAT",
    LOG_PREFIX + "[%(asctime)s][PID:%(process)d][%(levelname)s][%(name)s] %(message)s",
)
RQ_WORKER_JOB_LOG_FORMAT = os.environ.get(
    "DEEPBI_RQ_WORKER_JOB_LOG_FORMAT",
    (
...
# Mail settings:
MAIL_SERVER = os.environ.get("DEEPBI_MAIL_SERVER", "localhost")
MAIL_PORT = int(os.environ.get("DEEPBI_MAIL_PORT", 25))
MAIL_USE_TLS = parse_boolean(os.environ.get("DEEPBI_MAIL_USE_TLS", "false"))
MAIL_USE_SSL = parse_boolean(os.environ.get("DEEPBI_MAIL_USE_SSL", "false"))
MAIL_USERNAME = os.environ.get("DEEPBI_MAIL_USERNAME", None)
MAIL_PASSWORD = os.environ.get("DEEPBI_MAIL_PASSWORD", None)
MAIL_DEFAULT_SENDER = os.environ.get("DEEPBI_MAIL_DEFAULT_SENDER", None)
MAIL_MAX_EMAILS = os.environ.get("DEEPBI_MAIL_MAX_EMAILS", None)
MAIL_ASCII_ATTACHMENTS = parse_boolean(
    os.environ.get("DEEPBI_MAIL_ASCII_ATTACHMENTS", "false")
)
...
HOST = os.environ.get("DEEPBI_HOST", "")

SEND_FAILURE_EMAIL_INTERVAL = int(
    os.environ.get("DEEPBI_SEND_FAILURE_EMAIL_INTERVAL", 60)
)
MAX_FAILURE_REPORTS_PER_QUERY = int(
    os.environ.get("DEEPBI_MAX_FAILURE_REPORTS_PER_QUERY", 100)
)

ALERTS_DEFAULT_MAIL_SUBJECT_TEMPLATE = os.environ.get(
    "DEEPBI_ALERTS_DEFAULT_MAIL_SUBJECT_TEMPLATE", "({state}) {alert_name}"
)
...
RATELIMIT_ENABLED = parse_boolean(os.environ.get("DEEPBI_RATELIMIT_ENABLED", "true"))
THROTTLE_LOGIN_PATTERN = os.environ.get("DEEPBI_THROTTLE_LOGIN_PATTERN", "50/hour")
LIMITER_STORAGE = os.environ.get("DEEPBI_LIMITER_STORAGE", REDIS_URL)
THROTTLE_PASS_RESET_PATTERN = os.environ.get("DEEPBI_THROTTLE_PASS_RESET_PATTERN", "10/hour")

# CORS settings for the Query Result API (and possibly future external APIs).
# In most cases all you need to do is set DEEPBI_CORS_ACCESS_CONTROL_ALLOW_ORIGIN
# to the calling domain (or domains in a comma separated list).
ACCESS_CONTROL_ALLOW_ORIGIN = set_from_string(
    os.environ.get("DEEPBI_CORS_ACCESS_CONTROL_ALLOW_ORIGIN", "")
)
ACCESS_CONTROL_ALLOW_CREDENTIALS = parse_boolean(
    os.environ.get("DEEPBI_CORS_ACCESS_CONTROL_ALLOW_CREDENTIALS", "false")
)
ACCESS_CONTROL_REQUEST_METHOD = os.environ.get(
    "DEEPBI_CORS_ACCESS_CONTROL_REQUEST_METHOD", "GET, POST, PUT"
)
ACCESS_CONTROL_ALLOW_HEADERS = os.environ.get(
    "DEEPBI_CORS_ACCESS_CONTROL_ALLOW_HEADERS", "Content-Type"
)
...
enabled_query_runners = array_from_string(
    os.environ.get("DEEPBI_ENABLED_QUERY_RUNNERS", ",".join(default_query_runners))
)
additional_query_runners = array_from_string(
    os.environ.get("DEEPBI_ADDITIONAL_QUERY_RUNNERS", "")
)
disabled_query_runners = array_from_string(
    os.environ.get("DEEPBI_DISABLED_QUERY_RUNNERS", "")
)
...
dynamic_settings = importlib.import_module(
    os.environ.get("DEEPBI_DYNAMIC_SETTINGS_MODULE", "bi.settings.dynamic_settings")
)
...
enabled_destinations = array_from_string(
    os.environ.get("DEEPBI_ENABLED_DESTINATIONS", ",".join(default_destinations))
)
additional_destinations = array_from_string(
    os.environ.get("DEEPBI_ADDITIONAL_DESTINATIONS", "")
)

DESTINATIONS = distinct(enabled_destinations + additional_destinations)

EVENT_REPORTING_WEBHOOKS = array_from_string(
    os.environ.get("DEEPBI_EVENT_REPORTING_WEBHOOKS", "")
)

# Support for Sentry (https://getsentry.com/). Just set your Sentry DSN to enable it:
SENTRY_DSN = os.environ.get("DEEPBI_SENTRY_DSN", "")
SENTRY_ENVIRONMENT = os.environ.get("DEEPBI_SENTRY_ENVIRONMENT")

# Client side toggles:
ALLOW_SCRIPTS_IN_USER_INPUT = parse_boolean(
    os.environ.get("DEEPBI_ALLOW_SCRIPTS_IN_USER_INPUT", "false")
)
DASHBOARD_REFRESH_INTERVALS = list(
    map(
        ...
        array_from_string(
            os.environ.get(
                "DEEPBI_DASHBOARD_REFRESH_INTERVALS", "60,300,600,1800,3600,43200,86400"
            )
        ),
    )
)
...
TABLE_CELL_MAX_JSON_SIZE = int(os.environ.get("DEEPBI_TABLE_CELL_MAX_JSON_SIZE", 50000))

# Features:
VERSION_CHECK = parse_boolean(os.environ.get("DEEPBI_VERSION_CHECK", "true"))
FEATURE_DISABLE_REFRESH_QUERIES = parse_boolean(
    os.environ.get("DEEPBI_FEATURE_DISABLE_REFRESH_QUERIES", "false")
)
FEATURE_SHOW_QUERY_RESULTS_COUNT = parse_boolean(
    os.environ.get("DEEPBI_FEATURE_SHOW_QUERY_RESULTS_COUNT", "true")
)
FEATURE_ALLOW_CUSTOM_JS_VISUALIZATIONS = parse_boolean(
    os.environ.get("DEEPBI_FEATURE_ALLOW_CUSTOM_JS_VISUALIZATIONS", "false")
)
FEATURE_AUTO_PUBLISH_NAMED_QUERIES = parse_boolean(
    os.environ.get("DEEPBI_FEATURE_AUTO_PUBLISH_NAMED_QUERIES", "true")
)
FEATURE_EXTENDED_ALERT_OPTIONS = parse_boolean(
    os.environ.get("DEEPBI_FEATURE_EXTENDED_ALERT_OPTIONS", "false")
)

# BigQuery
BIGQUERY_HTTP_TIMEOUT = int(os.environ.get("DEEPBI_BIGQUERY_HTTP_TIMEOUT", "600"))

# Allow Parameters in Embeds
# WARNING: Deprecated!
ALLOW_PARAMETERS_IN_EMBEDS = parse_boolean(
    os.environ.get("DEEPBI_ALLOW_PARAMETERS_IN_EMBEDS", "false")
)

# Enhance schema fetching
SCHEMA_RUN_TABLE_SIZE_CALCULATIONS = parse_boolean(
    os.environ.get("DEEPBI_SCHEMA_RUN_TABLE_SIZE_CALCULATIONS", "false")
)

# kylin
KYLIN_OFFSET = int(os.environ.get("DEEPBI_KYLIN_OFFSET", 0))
KYLIN_LIMIT = int(os.environ.get("DEEPBI_KYLIN_LIMIT", 50000))
KYLIN_ACCEPT_PARTIAL = parse_boolean(
    os.environ.get("DEEPBI_KYLIN_ACCEPT_PARTIAL", "false")
)
...
REQUESTS_ALLOW_REDIRECTS = parse_boolean(
    os.environ.get("DEEPBI_REQUESTS_ALLOW_REDIRECTS", "false")
)
...
ENFORCE_CSRF = parse_boolean(
    os.environ.get("DEEPBI_ENFORCE_CSRF", "false")
)
