...
class ReportSqlserver(Report):
    async def deal_report(self, json_str, message):
        """ Process sqlserver data sources, generate reports, and select corresponding workflows """
        ...
        if q_sender == CONFIG.talker_user:
            ...
            if q_data_type == 'mysql_comment':
                ...
                if CONFIG.database_model == 'online':
                    ...
                    if if_suss:
                        self.agent_instance_util.base_sqlserver_info = '  When connecting to the database, be sure to bring the port. This is database info :' + '\n' + str(
                            db_info)
        ...
        try:
            ...
            for i in range(max_retry_times):
                try:
                    ...
                    sqlserver_engineer = self.agent_instance_util.get_agent_sqlserver_engineer()
                    bi_proxy = self.agent_instance_util.get_agent_bi_proxy()
                    chart_presenter = self.agent_instance_util.get_agent_chart_presenter()

                    agents = [sqlserver_engineer, bi_proxy, chart_presenter]
