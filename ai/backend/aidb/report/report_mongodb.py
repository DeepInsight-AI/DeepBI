...
class ReportMongoDB(Report):
    async def deal_report(self, json_str, message):
        """ Process mongodb data sources, generate reports, and select corresponding workflows """
        ...
        if q_sender == CONFIG.talker_user:
            ...
            if q_data_type == 'mysql_comment':
                ...
                if CONFIG.database_model == 'online':
                    ...
                    if if_suss:
                        self.agent_instance_util.base_mongodb_info = '  When connecting to the database, be sure to bring the port. This is database info :' + '\n' + str(
                            db_info)
