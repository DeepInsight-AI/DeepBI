...
def main(path, brand, cur_time, country):
    # 读取CSV文件
    file_path = r'C:\Users\admin\PycharmProjects\DeepBI\ai\backend\util\db\auto_yzj\滞销品优化\手动sp广告\商品投放搜索词优化\预处理.csv'
    ...
    condition_2 = (df['total_clicks_7d'] > 13) & (df['ORDER_7d'] == 0) & (df['total_cost_7d'] > 7)
    ...
    for index, row in result_df.iterrows():
        api.create_product_targets_search_term_info(country, brand, '滞销品优化', '手动_劣质', row['campaignName'],row['campaignId'],
