# filename: search_term_analysis_adjusted.py
...
# 定义调整后的条件
conditions = [
    (df['ACOS_30d'] > 0.2) & (df['ACOS_30d'] < 0.4) & (df['ORDER_1m'] <= 5),
    (df['ACOS_30d'] >= 0.4) & (df['ORDER_1m'] <= 10),
    (df['total_clicks_30d'] > 13) & (df['ORDER_1m'] == 0),
    (df['ACOS_7d'] > 0.24) & (df['ACOS_7d'] < 0.36) & (df['ORDER_7d'] <= 3),
    (df['ACOS_7d'] >= 0.36) & (df['ORDER_7d'] <= 5),
    (df['total_clicks_7d'] > 10) & (df['ORDER_7d'] == 0),
]

# 定义原因
reasons = [
    "最近一个月的平均ACOS值大于0.2小于0.4且订单数小于等于5",
    "最近一个月的平均ACOS值大于等于0.4且订单数小于等于10",
    "最近一个月的总点击次数大于13且订单数为0",
    "最近7天的ACOS值大于0.24小于0.36且订单数小于等于3",
    "最近7天的ACOS值大于等于0.36且订单数小于等于5",
    "最近7天的点击次数大于10且订单数为0",
]
