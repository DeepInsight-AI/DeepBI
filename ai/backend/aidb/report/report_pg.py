...
class ReportPostgresql(Report):
    async def deal_report(self, json_str, message):
        """ Process postgresql data sources, generate reports, and select corresponding workflows """
            ...
            if q_data_type == 'question':
                ...
                if self.agent_instance_util.base_message is not None:
                    try:
                        ...
                        # result['data']['content'] = await self.agent_instance_util.task_generate_report_postgresql(
                        #     q_str)
        ...
        try:
            ...
            for i in range(max_retry_times):
                try:
                    ...
                    postgresql_engineer = self.agent_instance_util.get_agent_postgresql_engineer()
                    bi_proxy = self.agent_instance_util.get_agent_bi_proxy()
                    chart_presenter = self.agent_instance_util.get_agent_chart_presenter()

                    agents = [postgresql_engineer, bi_proxy, chart_presenter]
