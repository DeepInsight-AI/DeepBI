...
def main(path, brand, cur_time, country, version=2):
    ...
    file_name = "手动_特殊商品投放" + '_' + brand + '_' + country + '_' + cur_time + '.csv'
    ...
    if version == 1:
        ...
        # 定义规则
        def apply_rules(row):
            if row['total_purchases7d_15d'] <= 2 and row['total_clicks_7d'] <= 3 and row['total_cost_7d'] < 7:
                return row['bid'] + 0.05, '定义一', 0.05
            elif row['total_purchases7d_15d'] <= 2 and 3 < row['total_clicks_7d'] <= 6 and row['total_cost_7d'] < 7:
                return row['bid'] + 0.04, '定义二', 0.04
            elif row['total_purchases7d_15d'] <= 2 and 6 < row['total_clicks_7d'] <= 9 and row['total_cost_7d'] < 7:
                return row['bid'] + 0.03, '定义三', 0.03
            elif row['total_purchases7d_15d'] <= 2 and 9 < row['total_clicks_7d'] <= 12 and row['total_cost_7d'] < 7:
                return row['bid'] + 0.02, '定义四', 0.02
            else:
                return row['bid'], '', ''
        ...
        filtered_data = filtered_data[[
            'campaignName', 'adGroupName', 'total_purchases7d_15d', 'total_clicks_7d', 'total_cost_7d',
            'keyword', 'bid', 'keywordId', 'New_Bid', 'reason', 'bid_adjust'
        ]]
        filtered_data.replace({np.nan: None}, inplace=True)
        api = DbNewSpTools(brand,country)
        for index, row in filtered_data.iterrows():
            api.create_product_targets_info(country, brand, '日常优化', '手动_特殊', row['keyword'],
                                                row['keywordId'],
                                                row['campaignName'], row['adGroupName'],None, row['bid'],
                                                row['New_Bid'], None, None,
                                                None, None,None,row['total_purchases7d_15d'],
                                                None, row['total_clicks_7d'], None, row['total_cost_7d'],
