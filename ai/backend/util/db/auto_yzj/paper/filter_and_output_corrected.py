# filename: filter_and_output_corrected.py
import pandas as pd
...
# 计算新的预算
filtered_data['new_Budget'] = filtered_data.apply(lambda row: max(8, row['Budget'] - 5) if 'new_Budget' not in row.name else row['Budget'], axis=1)

# 输出结果到CSV文件
output_columns = ['date', 'campaignName', 'Budget', 'clicks', 'ACOS', 'avg_ACOS_7d', 'clicks_7d', 'sales_7d', 'avg_ACOS_1m', 'clicks_1m', 'sales_1m', 'country_avg_ACOS_1m', 'new_Budget', 'reason']
