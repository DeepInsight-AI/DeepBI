...
def main(path, brand, cur_time, country, version=2):
    ...
    # 筛选满足任意一个定义条件的sku
        ...
        # 定义筛选条件
        condition1 = (df['ORDER_1m'] < 6) & (df['ACOS_7d'] > 0.24) & (df['total_clicks_7d'] > 13) & (df['total_sales14d_7d'] == 0) & (df['total_cost_7d'] > 5)
        ...
        condition5 = (df['ORDER_1m'] < 6) & (df['total_clicks_7d'] >= 19) & (df['total_sales14d_7d'] == 0) & (df['total_cost_7d'] > 8)
        condition6 = (df['ORDER_1m'] >= 6) & (df['total_clicks_7d'] >= 26) & (df['total_sales14d_7d'] == 0) & (df['total_cost_7d'] > 10)
    ...
    for index, row in result_df.iterrows():
        api.create_sku_info(country, brand, '日常优化', '手动_关闭', row['campaignName'],row['adGroupName'],
...
#main('C:/Users/admin/PycharmProjects/DeepBI/ai/backend/util/db/auto_yzj/日常优化/输出结果/LAPASA_US_2024-07-12','LAPASA','2024-07-12','US')
