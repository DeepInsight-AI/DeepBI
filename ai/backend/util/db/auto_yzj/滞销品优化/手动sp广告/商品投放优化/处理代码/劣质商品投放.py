...
def main(path, brand, cur_time, country, version: int = 1):
    ...
    def adjust_bid(row, cause):
        ...
        new_bid = keywordBid / ((avg_ACOS_7d - 0.27) / 0.27 + 1)
    ...
    for idx, row in data.iterrows():
        ...
        if 0.27 < avg_ACOS_7d <= 0.5 and 0 < avg_ACOS_30d <= 0.5 and order_1m < 5 and avg_ACOS_3d >= 0.27:
            new_bid, cause = adjust_bid(row, "定义一")
            bid_adjust = 0
        elif avg_ACOS_7d > 0.5 and avg_ACOS_30d <= 0.36 and avg_ACOS_3d > 0.27:
            new_bid, cause = adjust_bid(row, "定义二")
            bid_adjust = 0
        elif total_clicks_7d >= 10 and total_sales14d_7d == 0 and total_cost_7d <= 7 and avg_ACOS_30d <= 0.36:
            ...
            cause = "定义四"
            bid_adjust = -1
        elif avg_ACOS_7d > 0.5 and avg_ACOS_3d > 0.27 and avg_ACOS_30d > 0.36:
            new_bid = 0.05
            cause = "定义五"
            bid_adjust = -1
            ...
            cause = "定义六"
            bid_adjust = -1
        elif 0.27 < avg_ACOS_7d <= 0.5 and 0 < avg_ACOS_30d <= 0.5 and order_1m < 5 and total_sales14d_3d == 0:
            ...
            bid_adjust = -1
            cause = "定义九"
        elif 0.27 < avg_ACOS_7d <= 0.5 and avg_ACOS_30d > 0.5 and order_1m < 5 and avg_ACOS_3d >= 0.27:
            new_bid, cause = adjust_bid(row, "定义十")
            bid_adjust = 0
        elif 0.27 < avg_ACOS_7d <= 0.5 and total_sales14d_3d == 0 and avg_ACOS_30d > 0.5:
            new_bid, cause = adjust_bid(row, "定义十一")
            bid_adjust = 0
        elif avg_ACOS_7d <= 0.27 and total_sales14d_3d == 0 and total_cost_3d > 5:
            new_bid = max(keyword_bid - 0.01, 0.05)
            cause = "定义十二"
            bid_adjust = -0.01
        elif avg_ACOS_7d <= 0.27 and 0.27 < avg_ACOS_3d:
            new_bid = max(keyword_bid - 0.01, 0.05)
            cause = "定义十三"
            bid_adjust = -0.01
        elif total_clicks_7d >= 10 and total_sales14d_7d == 0 and total_cost_7d > 7 and avg_ACOS_30d >= 0.36:
    ...
    for index, row in results_df.iterrows():
        api.create_product_targets_info(country, brand, '滞销品优化', '手动_劣质', row['keyword'], row['keywordId'],
