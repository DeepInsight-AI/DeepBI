...
def main(path, brand, cur_time, country):
    # 读取CSV文件路径
    csv_path = r"C:\Users\admin\PycharmProjects\DeepBI\ai\backend\util\db\auto_yzj\日常优化\手动sp广告\复开SKU\预处理.csv"
    file_name = "手动_复开SKU" + '_' + brand + '_' + country + '_' + cur_time + '.csv'
    ...
    filtered_data['Reason'] = filtered_data.apply(
        lambda row: 'Definition1' if row['ACOS_30d'] > 0 and row['ACOS_30d'] <= 0.24 and row['ACOS_7d'] > 0 and row[
            'ACOS_7d'] <= 0.24
        else 'Definition2' if row['ACOS_30d'] > 0 and row['ACOS_30d'] <= 0.24 and row['total_clicks_7d'] == 0
        else '', axis=1)
    ...
    for index, row in output_data.iterrows():
        api.create_sku_info(country, brand, '日常优化', '手动_复开', row['campaignName'], row['adGroupName'],
