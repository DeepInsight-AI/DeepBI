...
def detect_anomalies(row):
    ...
    if pd.isna(acos_yesterday) or acos_yesterday == 0:
        if acos_7d < 0.25 or acos_30d < 0.25:
            anomalies.append(f"昨天无销售额，近7天ACOS较好" if acos_7d < 0.25 else "")
            anomalies.append(f"昨天无销售额，近30天ACOS较好" if acos_30d < 0.25 else "")
        if acos_7d < 0.20 or acos_30d < 0.20:
            anomalies.append(f"昨天无销售额，近7天ACOS极好" if acos_7d < 0.20 else "")
            anomalies.append(f"昨天无销售额，近30天ACOS极好" if acos_30d < 0.20 else "")
