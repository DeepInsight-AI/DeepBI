...
def main(path, brand, cur_time, country, db, version=2):
    # 读取CSV文件
    file_path = os.path.join(get_auto_path(), '日常优化\手动sp广告\商品投放搜索词优化\预处理.csv')
    ...
    required_columns = [
        ...
        'ORDER_7d', 'total_clicks_7d', 'total_cost_7d', 'CPC_30d', 'searchTerm', 'reason'
    ]
    result_df = filtered_df[required_columns]
    # 过滤长度为10位并且以'b0'开头的行
    result_df = result_df[(result_df['searchTerm'].str.len() == 10) & (result_df['searchTerm'].str.startswith('b0'))]
