...
def main(path, brand, cur_time, country, version=2):
    # 文件路径定义
    input_csv_path = r'C:\Users\admin\PycharmProjects\DeepBI\ai\backend\util\db\auto_yzj\日常优化\自动sp广告\SKU优化\预处理.csv'
    file_name = "自动_关闭SKU" + '_' + brand + '_' + country + '_' + cur_time + '.csv'
    ...
    if version == 1:
        ...
        condition2 = (df['ORDER_1m'] < 8) & (df['ACOS_30d'] > 0.24) & (df['total_sales14d_7d'] == 0) & (
                df['total_clicks_7d'] > 13)
        condition3 = (df['ORDER_1m'] < 8) & (df['ACOS_7d'].between(0.24, 0.5)) & (df['ACOS_30d'].between(0, 0.24)) & (
                df['total_clicks_7d'] > 13)
        ...
        # 定义筛选条件
        condition1 = (df['ORDER_1m'] < 6) & (df['ACOS_7d'] > 0.24) & (df['total_clicks_7d'] > 13) & (
                df['total_sales14d_7d'] == 0) & (df['total_cost_7d'] > 5)
        condition2 = (df['ORDER_1m'] < 6) & (df['ACOS_30d'] > 0.24) & (df['ACOS_7d'] > 0.36)
        condition3 = df['ACOS_7d'] > 0.5
        condition4 = (df['total_clicks_30d'] > 13) & (df['total_cost_30d'] > 8) & (df['total_sales14d_30d'] == 0)
        condition5 = (df['ORDER_1m'] < 6) & (df['total_clicks_7d'] >= 19) & (df['total_sales14d_7d'] == 0) & (
                df['total_cost_7d'] > 8)
        condition6 = (df['ORDER_1m'] >= 6) & (df['total_clicks_7d'] >= 26) & (df['total_sales14d_7d'] == 0) & (
                df['total_cost_7d'] > 10)
    ...
    # 选择指定列
    output_columns = [
        'campaignName', 'adGroupName', 'adId', 'ACOS_30d', 'total_clicks_30d','total_sales14d_30d','total_cost_30d',
        'ACOS_7d', 'total_clicks_7d','total_sales14d_7d','total_cost_7d',
    ...
    for index, row in result_df.iterrows():
        api.create_sku_info(country, brand, '日常优化', '自动_关闭', row['campaignName'],row['adGroupName'],
                               row['adId'], row['ACOS_30d'], row['total_clicks_30d'],
                               row['total_sales14d_30d'], row['total_cost_30d'], row['ACOS_7d'], row['total_clicks_7d'], row['total_sales14d_7d'],
