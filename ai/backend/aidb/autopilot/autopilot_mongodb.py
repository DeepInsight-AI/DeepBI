...
class AutopilotMongoDB(Autopilot):
    ...
    async def task_base(self, qustion_message):
        """ Task type: mongodb data analysis"""
        try:
            error_times = 0
            for i in range(max_retry_times):
                try:
                    base_mongodb_assistant = self.get_agent_base_mongodb_assistant()
                    python_executor = self.agent_instance_util.get_agent_python_executor()

                    await python_executor.initiate_chat(
                        base_mongodb_assistant,
                        message=self.agent_instance_util.base_message + '\n' + self.question_ask + '\n' + str(
                            qustion_message),
                    )

                    answer_message = python_executor.chat_messages[base_mongodb_assistant]
