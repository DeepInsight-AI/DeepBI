...
from bi import redis_connection, utils, settings
from bi.destinations import (
    get_configuration_schema_for_destination_type,
    get_destination,
)
from bi.metrics import database  # noqa: F401
...
class DataSource(BelongsToOrgMixin, db.Model):
    ...
    def get_private_options(self):  # add at 10.30 17:05
    ...
    def create_with_group(cls, *args, **kwargs):
        ...
        print(data_source.org.default_group,"data_source.org.default_group")
    ...
    def _sort_schema(self, schema):
        ...
        for i in sorted(schema, key=lambda x: x["name"]):
            if 'comment' in i:
                combined = list(zip(i['columns'], i['comment']))
                combined.sort()
                i['columns'], i['comment'] = zip(*combined)
                result.append({
                    "name": i["name"],
                    "columns": i['columns'],
                    "comment": i['comment']
...
class Query(ChangeTrackingMixin, TimestampMixin, BelongsToOrgMixin, db.Model):
    ...
    def create(cls, **kwargs):
        ...
        db.session.add(
            Visualization(
                query_rel=query,
                name="表格" if settings.WEB_LANGUAGE == 'CN' else 'Table',
                description="",
                type="TABLE",
                options="{}",
            )
...
# new table opt
@generic_repr("id", "table_name", "table_inuse", "table_columns_info", "data_source_id", "org_id")
class DataTableColumns(BelongsToOrgMixin, db.Model):
    id = primary_key("DataTableColumns")
    table_name = Column(db.String(255))
    table_desc = Column(db.Text)
    table_inuse = Column(db.Boolean, default=True)
    table_columns_info = Column(db.Text)
    data_source_id = Column(db.Integer, nullable=False)
    # data_source_id = Column(key_type("DataSource"), db.ForeignKey("data_sources.id"))
    # data_source = db.relationship(DataSource, back_populates="data_source_columns")
    org_id = Column(key_type("Organization"), db.ForeignKey("organizations.id"))
    organization = db.relationship(Organization, back_populates="data_table_columns")

    __tablename__ = "data_table_columns"
    __table_args__ = (db.Index("db_id_table_name", "table_name", "data_source_id", unique=True),)

    @classmethod
    def table_info(cls, data_source_id, table_name):
        return cls.query.filter(cls.table_name == table_name,
                                cls.data_source_id == data_source_id).one()

    @classmethod
    def have_it(cls, data_source_id, table_name):
        return cls.query.filter(cls.table_name == table_name,
                                cls.data_source_id == data_source_id).count() > 0
    ...
    def to_dict(self):
        return {
            ...
            "table_inuse": self.table_inuse,
            "table_columns_info": json_loads(self.table_columns_info),
            "data_source_id": self.data_source_id,
            "org_id": self.org_id
        }
...
# new file table
@generic_repr("id", "user_id", "org_id", "source_name", "file_name", "is_use", "file_type", "created_at")
class DataSourceFile(BelongsToOrgMixin, db.Model):
    id = primary_key("DataSourceFile")
    user_id = Column(key_type("User"), db.ForeignKey("users.id"))
    users = db.relationship(User, backref="date_source_file")
    org_id = Column(key_type("Organization"), db.ForeignKey("organizations.id"))
    organizations = db.relationship(Organization, backref="date_source_file")
    source_name = Column(db.String(255))
    file_name = Column(db.String(255))
    is_use = Column(db.Boolean, default=True)
    file_type = Column(db.String(255))
    created_at = Column(db.DateTime(True), default=db.func.now())

    __tablename__ = "data_source_file"
    __table_args__ = (db.Index("file_id_table_name", "user_id", "source_name", unique=True),)
    ...
    def check_have_name(cls, source_name, user_id):
        return cls.query.filter(cls.user_id == user_id,
                                cls.source_name == source_name).count() > 0
...
# new AutoPilot table
@generic_repr("id", "user_id", "org_id", "report_name", "file_name", "is_use", "created_at", "is_generate")
class DataReportFile(BelongsToOrgMixin, db.Model):
    ...
    __table_args__ = (db.Index("file_id_report_name", "user_id", "file_name", unique=True),)
