# filename: process_poor_performance_campaigns_v3.py
...
new_budgets = []
reasons = []
for index, row in poor_performance_campaigns.iterrows():
    if condition1[index]:
        new_budget = max(row['Budget'] - 5, 8)
        reason = '定义一'
    elif condition2[index]:
        new_budget = max(row['Budget'] - 5, 8)
        reason = '定义二'
    elif condition3[index]:
...
poor_performance_campaigns['new_budget'] = new_budgets
poor_performance_campaigns['reason'] = reasons
