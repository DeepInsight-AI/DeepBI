# filename: adjust_bids_debug_v5.py
...
for campaign in campaigns:
    ...
    for placement in ['Top of Search on-Amazon', 'Detail Page on-Amazon', 'Other on-Amazon']:
        ...
        # Print placement stats for debugging
        print(f"\nStatistics for Campaign: {campaign}, Placement: {placement}")
        print(placement_df[['ACOS_7d', 'ACOS_3d', 'total_clicks_7d', 'total_clicks_3d', 'total_cost_3d']].describe())
        
        # Check conditions for Definition 1
        condition1 = (placement_df['ACOS_7d'] > 0) & (placement_df['ACOS_7d'] <= 0.35) & \
                     (placement_df['total_clicks_7d'] < placement_df['total_clicks_7d'].max()) & \
                     (placement_df['ACOS_3d'] > 0) & (placement_df['ACOS_3d'] <= 0.35) & \
        ...
        condition2 = (placement_df['ACOS_7d'] > 0) & (placement_df['ACOS_7d'] <= 0.35) & \
                     (placement_df['total_clicks_7d'] < placement_df['total_clicks_7d'].max()) & \
                     (placement_df['total_cost_3d'] < 5) & \
                     (placement_df['total_clicks_3d'] < placement_df['total_clicks_3d'].max())
