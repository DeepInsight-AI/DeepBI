...
class BudgetRulesToolsSD(BaseApi):
    ...
    def create_budget_rules_api(self,campaign_info):
        try:
            result = sponsored_display.BudgetRules(credentials=self.credentials,
                                                    marketplace=Marketplaces[self.market.upper()],
                                                    access_token=self.access_token,
                                                    proxies=get_proxies(self.market),
...
if __name__ == "__main__":
    BudgetRulesToolsSD('LAPASA').Associates_budget_rules_to_campaign('da16a837-cd35-4baa-988c-84788f5bb726',540943190583757,'IT')
