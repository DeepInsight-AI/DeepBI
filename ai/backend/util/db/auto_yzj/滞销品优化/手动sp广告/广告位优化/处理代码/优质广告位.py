...
def main(path, brand, cur_time, country, version=2):
    ...
    file_path = r'C:\Users\admin\PycharmProjects\DeepBI\ai\backend\util\db\auto_yzj\滞销品优化\手动sp广告\广告位优化\预处理.csv'
    file_name = "手动_优质广告位" + '_' + brand + '_' + country + '_' + cur_time + '.csv'
    ...
    if version == 1:
        ...
        for index, row in data.iterrows():
            ...
            bid = row['bid']
            if 0 < ACOS_7d < 0.27 and 0 < ACOS_3d < 0.27:
                reason = '定义一'
                bid_adjust = 5
            ...
            elif 0 < ACOS_7d < 0.27 and total_sales14d_3d == 0 and total_cost_3d < 5:
                reason = '定义二'
                bid_adjust = 5
                new_bid = min(50, row['bid'] + 5)
    ...
    for index, row in result_df.iterrows():
        api.create_campaign_placement_info(country,brand,'滞销品优化','手动_优质',row['campaignName'],row['campaignId'],row['placementClassification'],row['bid'],row['new_bid'],row['ACOS_7d'],None,None,row['ACOS_3d'],row['total_sales14d_3d'],row['total_cost_3d'],row['reason'],row['bid_adjust'],cur_time,datetime.now(),0)
