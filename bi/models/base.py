...
class BiSQLAlchemy(SQLAlchemy):
    def apply_driver_hacks(self, app, info, options):
        ...
        super(BiSQLAlchemy, self).apply_driver_hacks(app, info, options)

    def apply_pool_defaults(self, app, options):
        super(BiSQLAlchemy, self).apply_pool_defaults(app, options)
        if settings.SQLALCHEMY_ENABLE_POOL_PRE_PING:
...
db = BiSQLAlchemy(session_options={"expire_on_commit": False})
