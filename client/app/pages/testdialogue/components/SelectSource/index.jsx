...
import csv from "@/assets/images/db-logos/excel.png";
...
const SelectSource = forwardRef(({ confirmLoading, Holmestable, chat_type, onChange, onSuccess,percent },ref) => {
  ...
  const getTableColumns = async (item,type=null) => {
    ...
    const existingTableSelectedRowKeys = tableSelectedRowKeys.find(table => table.table_name === table_desc_obj.table_name);
    if (existingTableSelectedRowKeys) {
      ...
      setTableSelectedRowKeys(prevData => [...prevData, { table_name: table_desc_obj.table_name, selectedRowKeys: newSelectedRowKeys }]);
    }
  ...
  const rowSelection = {
    ...
    onChange: (selectedRowKeys, selectedRows) => {
      ...
      setSchemaListData(newSchemaListDataList);
      const existingTableSelectedRowKeys = tableSelectedRowKeys.find(table => table.table_name === newSchemaListData.table_name);
      if (existingTableSelectedRowKeys) {
        setTableSelectedRowKeys(tableSelectedRowKeys.map(table => {
        ...
        setTableSelectedRowKeys(prevData => [...prevData, { table_name: newSchemaListData.table_name, selectedRowKeys: selectedRowKeys }]);
      }
    },
  };
    ...
    !Holmestable && (
      <div className="flex-column">
        ...
        <Spin spinning={SelectLoading} className={!submitting?"":"dislogue-spin"}>
          ...
          <SecondChoice SelectLoading={SelectLoading} editData={editData} closeEditData={closeEditData} submit={submit} source_item={source_item}></SecondChoice>
        :
          <div className="select-content-item">
            ...
            <div style={{ display: "flex", alignItems: "baseline" }}>
              <span className="select-content-item-span">{window.W_L.data_source}
                  ...
                  <ul className={!SchemaListIsShow ? "flex-center" : ""}>
                   {
                      ...
                      <div className="flex-end">
                        ...
                        <Checkbox indeterminate={indeterminate} checked={checkAll} onChange={(e) => changeSourceAll(e,"all")} />
                      </div>
                      )
                   }
            ...
            {selectSchema && selectSchema.length > 0 && (
              <button onClick={submit} disabled={SelectLoading} className="submit-btn">{window.W_L.submit} {selectSchema.length}</button>
            )}
          </div>
          }
        </Spin>
