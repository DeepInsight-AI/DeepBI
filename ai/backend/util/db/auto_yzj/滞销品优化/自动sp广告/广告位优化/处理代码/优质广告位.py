...
def main(path, brand, cur_time, country, version=2):
    ...
    file_path = r'C:\Users\admin\PycharmProjects\DeepBI\ai\backend\util\db\auto_yzj\滞销品优化\自动sp广告\广告位优化\预处理.csv'
    ...
    for index, row in result_df.iterrows():
        api.create_campaign_placement_info(country,brand,'滞销品优化','自动_优质',row['campaignName'],row['campaignId'],row['placementClassification'],row['bid'],row['new_bid'],row['ACOS_7d'],None,None,row['ACOS_3d'],row['total_sales14d_3d'],row['total_cost_3d'],row['reason'],row['bid_adjust'],cur_time,datetime.now(),0)
