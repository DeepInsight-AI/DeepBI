# filename: adjust_bids_debug_v3.py
...
for campaign in campaigns:
    ...
    for placement in ['Top of Search on-Amazon', 'Detail Page on-Amazon', 'Other on-Amazon']:
        ...
        condition1 = (placement_df['ACOS_7d'] > 0) & (placement_df['ACOS_7d'] <= 0.3) & \
                     (placement_df['total_clicks_7d'] < placement_df['total_clicks_7d'].max()) & \
                     (placement_df['ACOS_3d'] > 0) & (placement_df['ACOS_3d'] <= 0.3) & \
        ...
        condition2 = (placement_df['ACOS_7d'] > 0) & (placement_df['ACOS_7d'] <= 0.3) & \
                     (placement_df['total_clicks_7d'] < placement_df['total_clicks_7d'].max()) & \
                     (placement_df['total_cost_3d'] < 4.5) & \
                     (placement_df['total_clicks_3d'] < placement_df['total_clicks_3d'].max())
