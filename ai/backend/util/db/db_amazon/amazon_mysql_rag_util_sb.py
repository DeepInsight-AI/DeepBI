...
class AmazonMysqlSBRagUitl:
    ...
    #  SB
    def get_sb_keyword_11(self, market, startdate , enddate):
        """关键词优化分析：-1-1.1  在 2024.04.01 至 2024.04.14 这段时间内，  法国SB广告中ACOS值低于18%  ，并且没有在广告定向投放中出现的用户搜索关键词。 将这些关键词信息生成csv文件，里面记录这些关键词的以下信息，CPC，SKU/ASIN， ACOS, Clicks."""
        try:
            conn = self.conn
            ...
            SELECT
    keywordId,
    IF(sum(sales) > 0, (sum(cost) / sum(sales)), NULL) AS ACOS
FROM
    amazon_search_term_reports_sb
WHERE
    DATE BETWEEN '{}' AND '{}' and market = '{}'
...
HAVING
    ACOS < 0.18""".format(startdate, enddate,market,startdate, enddate,market )
            df = pd.read_sql(query, con=conn)
            # return df
            output_filename = get_timestamp() + '_SB_keyword_1_1.csv'
            df.to_csv(output_filename, index=False, encoding='utf-8-sig')
            s1 = f"关键词数量：{df['keywordId'].drop_duplicates().shape[0]}"
            result_str = "[ACOS取值为0.2][{}]生成文件为：{}".format(s1,output_filename)
            print("1.1 Data inserted successfully!")
            return result_str
        except Exception as error:
            print("1.1 Error while inserting data:", error)

    def get_sb_keyword_121(self, market, startdate, enddate):
        """关键词优化分析：-1-2.1  1.在 2024.04.01 至 2024.04.14 这段时间内，  法国SB广告中ACOS值低于14%的定向投放关键词。将这些关键词信息生成csv文件，里面记录这些关键词的以下信息，CPC，SKU/ASIN， ACOS, Clicks，adgroupid.(ACOS20%的-30%) 关键词 提高出价 10%"""
        try:
            conn = self.conn
...
GROUP BY
        ...
        keywordText,
                                ...
                                adGroupName
HAVING
        ( SUM( cost ) / SUM( sales ) ) < 0.14
            """.format(market,startdate, enddate)
            df = pd.read_sql(query, con=conn)
            # return df
            output_filename = get_timestamp() + '_SB_keyword_2_1.csv'
            df.to_csv(output_filename, index=False, encoding='utf-8-sig')
...
HAVING
    ...
    def get_sb_keyword_131(self, market, startdate, enddate):
        """关键词优化分析：-1-3.1  1.在 2024.04.01 至 2024.04.14 这段时间内，  法国SB广告中ACOS值大于26%的 定向投放关键词。将这些关键词信息生成csv文件，里面记录这些关键词的以下信息，CPC，SKU/ASIN， ACOS,  Clicks，adgroupid.(ACOS20%的+30%)"""
        try:
            conn = self.conn
...
GROUP BY
        ...
        keywordText,
                                ...
                                adGroupName
HAVING
        ( SUM( cost ) / SUM( sales ) ) > 0.26
            """.format(market,startdate, enddate)
            df = pd.read_sql(query, con=conn)
            # return df
            output_filename = get_timestamp() + '_SB_keyword_3_1.csv'
            df.to_csv(output_filename, index=False, encoding='utf-8-sig')
            s1 = f"关键词数量：{df[['campaignName','adGroupName','keywordBid','keywordText']].drop_duplicates().shape[0]}"
            result_str = "[ACOS取值为0.2][{}]生成文件为：{}".format(s1, output_filename)
...
HAVING
    ...
    def get_sb_advertise_311(self, market, startdate, enddate):
        """广告优化分析：-3-1.1  1.在 2024.04.01 至 2024.04.14 这段时间内，  法国SB广告中ACOS值低于14%的  campaign 广告活动。将这些信息生成csv文件，里面记录这些关键词的以下信息，CPC， ACOS,  Clicks，campaignid，spend.（-30%）"""
        try:
            conn = self.conn
