...
class AnalysisSqlserver(Analysis):
    async def deal_question(self, json_str, message):
        """ Process the sqlserver data source and select the corresponding workflow """
    ...
    async def task_base(self, qustion_message):
        ...
        try:
            ...
            for i in range(max_retry_times):
                try:
                    base_mysql_assistant = self.get_agent_base_sqlserver_assistant()
    ...
    async def task_generate_echart(self, qustion_message):
        """ Task type: sqlserver echart code block"""
        try:
            base_content = []
            base_mess = []
            report_demand_list = []
