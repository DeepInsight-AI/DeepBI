...
# 选择需要的列并添加定义列
result_df['定义'] = result_df.apply(lambda row: '定义一' if (row['ACOS_30d'] <= 0.24 and row['ACOS_7d'] <= 0.24) else '定义二', axis=1)
output_df = result_df[['campaignName', 'adId', 'adGroupName', 'ACOS_30d', 'ACOS_7d', 'total_clicks_7d', 'advertisedSku', 'ORDER_1m', '定义']]

# 保存结果到CSV文件
output_file_path = r'C:\Users\admin\PycharmProjects\DeepBI\ai\backend\util\db\auto_yzj\日常优化\手动sp广告\复开SKU\提问策略\批量复开SKU_v1_1_LAPASA_IT_2024-07-02.csv'
