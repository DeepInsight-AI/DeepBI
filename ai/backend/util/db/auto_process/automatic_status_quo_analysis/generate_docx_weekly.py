...
def generate_docx(db, brand, market,start_date,end_date,period):
    ...
    doc.add_heading(f'1.3 本周目标及实际（{date2_date}-{date1_date}）：', 3)
    ...
    doc.add_heading('3.1 本周总结（整体总结）', 3)
    doc.add_heading('3.2 下周优化策略', 3)
    doc.add_heading('3.3 下周计划与目标', 3)
    doc.add_heading('3.3.1 下周目标设定', 4)
    ...
    docx_path = f'{brand}_{market}_2024年{week_number}周_({date2_date}-{date1_date}).docx'
