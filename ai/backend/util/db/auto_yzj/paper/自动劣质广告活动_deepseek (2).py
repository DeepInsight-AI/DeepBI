# filename: analyze_ad_campaigns_optimized.py
import pandas as pd

# 读取CSV文件，使用较低的内存模式
data = pd.read_csv(r'C:\Users\admin\PycharmProjects\DeepBI\ai\backend\util\db\auto_yzj\日常优化\自动sp广告\预算优化\预处理.csv', low_memory=False)
...
for index, row in data.iterrows():
    if row['Condition'] == '定义一' or row['Condition'] == '定义二':
        while row['Budget'] > 8:
            data.at[index, 'Budget'] -= 5
    elif row['Condition'] == '定义三':
        while row['Budget'] > 5:
            data.at[index, 'Budget'] -= 5
    elif row['Condition'] == '定义四':
        data.at[index, 'Budget'] = '关闭'
...
data[['campaignId', 'campaignName', 'Budget', 'clicks_yesterday', 'ACOS_yesterday', 'ACOS_7d', 'total_clicks_7d', 'total_sales14d_7d', 'ACOS_30d', 'total_clicks_30d', 'total_sales14d_30d', 'country_avg_ACOS_1m', 'Condition']].to_csv(output_path, index=False)
