...
FROM (
    ...
    WHERE
        a.date BETWEEN DATE_SUB('{cur_time}', INTERVAL 30 DAY) AND '{cur_time}'- INTERVAL 1 DAY -- 选择过去30天的数据，包括'{cur_time}'
        AND a.market = '{country}'
        AND a.campaignId IN (
            SELECT campaignId
            FROM amazon_advertised_product_reports_sp
            WHERE campaignStatus = 'ENABLED' AND date = '{cur_time}' - INTERVAL 1 DAY -- 选择广告状态为 'ENABLED' 且日期为 '{cur_time}' 前一天的 adId
             )     AND c.targetingType like '%AUT%' -- 筛选出自动广告
...
WHERE p.state = 'PAUSED'; -- 只保留state为‘PAUSED’的数据项
                        """
        return query
