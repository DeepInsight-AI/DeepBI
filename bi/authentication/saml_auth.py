...
from bi.authentication import create_and_login_user, logout_and_redirect_to_index
from bi.authentication.org_resolving import current_org
from bi.handlers.base import org_scoped_rule
from bi.utils import mustache_render
...
def idp_initiated(org_slug=None):
    ...
    try:
        ...
        return redirect(url_for("bi.login", org_slug=org_slug))
    ...
    if "BiGroups" in authn_response.ava:
        group_names = authn_response.ava.get("BiGroups")
        user.update_group_assignments(group_names)

    url = url_for("bi.index", org_slug=org_slug)
