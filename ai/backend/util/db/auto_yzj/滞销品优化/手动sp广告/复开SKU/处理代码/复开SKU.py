...
def main(path, brand, cur_time, country, version=2):
    # 读取CSV文件路径
    csv_path = r"C:\Users\admin\PycharmProjects\DeepBI\ai\backend\util\db\auto_yzj\滞销品优化\手动sp广告\复开SKU\预处理.csv"
    ...
    if version == 1:
        ...
        condition1 = (data['ACOS_30d'] > 0) & (data['ACOS_30d'] <= 0.27) & (data['ACOS_7d'] > 0) & (data['ACOS_7d'] <= 0.27)
    ...
    for index, row in output_data.iterrows():
        api.create_sku_info(country, brand, '滞销品优化', '手动_复开', row['campaignName'], row['adGroupName'],
