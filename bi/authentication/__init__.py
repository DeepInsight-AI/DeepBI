...
from bi.settings.organization import settings as org_settings
from bi.tasks import record_event
...
def logout_and_redirect_to_index():
    ...
    if settings.MULTI_ORG and current_org == None:
        ...
        index_url = url_for("bi.index", org_slug=current_org.slug, _external=False)
    else:
        index_url = url_for("bi.index", _external=False)

    return redirect(index_url)


def init_app(app):
    from bi.authentication import (
        saml_auth,
        remote_user_auth,
        ldap_auth,
    )

    from bi.authentication.google_oauth import create_google_oauth_blueprint
