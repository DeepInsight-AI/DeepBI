...
def main(path, brand, cur_time, country, version: int = 3):
    ...
    if version == 1:
        ...
        for index, row in data.iterrows():
            ...
            if row['ACOS_7d'] > 0.5:
                if row['ACOS_30d'] > 0.24:
                    keyword_info["New_keywordBid"] = "关闭"
                    keyword_info["Reason"] += "定义八"
                elif 0 < row['ACOS_30d'] < 0.24:
                    keyword_info["New_keywordBid"] = row['keywordBid'] - 0.05
        ...
        for index, row in data.iterrows():
            ...
            if 0.24 < row['ACOS_7d'] < 0.5 and 0 < row['ACOS_30d'] < 0.24 and row['ACOS_3d'] >= 0.24:
                new_keywordBid = max(0.05, row['keywordBid'] - 0.03) if row['keywordBid'] >= 0.05 else \
                row['keywordBid']
                bid_adjust = -0.03
                action_reason = "定义一：更新出价"

            elif 0.24 < row['ACOS_7d'] < 0.5 and 0.24 < row['ACOS_30d'] < 0.5 and row['ACOS_3d'] >= 0.24:
                new_keywordBid = max(0.05, row['keywordBid'] - 0.04) if row['keywordBid'] >= 0.05 else \
                row['keywordBid']
                bid_adjust = -0.04
                action_reason = "定义二：更新出价"

            elif row['total_clicks_7d'] > 13 and row['total_sales14d_7d'] == 0 and 0.24 < row['ACOS_30d'] < 0.5 and 5 < row[
                'total_cost_7d'] < 8:
                new_keywordBid = max(0.05, row['keywordBid'] - 0.04) if row['keywordBid'] >= 0.05 else row['keywordBid']
                bid_adjust = -0.04
                action_reason = "定义三：降低出价"

            elif 0.24 < row['ACOS_7d'] < 0.5 and row['ACOS_3d'] >= 0.24 and row['ACOS_30d'] > 0.5:
                new_keywordBid = max(0.05, row['keywordBid'] - 0.05) if row['keywordBid'] >= 0.05 else row['keywordBid']
                bid_adjust = -0.05
                action_reason = "定义四：降低出价"

            elif row['ACOS_7d'] > 0.5 and 0 < row['ACOS_30d'] < 0.24 and row['ACOS_3d'] >= 0.24:
                new_keywordBid = max(0.05, row['keywordBid'] - 0.05) if row['keywordBid'] >= 0.05 else row['keywordBid']
                bid_adjust = -0.05
                action_reason = "定义五：降低出价"

            elif row['total_sales14d_30d'] == 0 and row['total_cost_30d'] > 7 and row['total_clicks_30d'] > 13:
                new_keywordBid = 0.05
                bid_adjust = -1
            ...
            elif row['ACOS_7d'] > 0.5 and row['ACOS_30d'] > 0.24 and row['ACOS_30d'] >= 0.24:
                new_keywordBid = 0.05
                bid_adjust = -1
                action_reason = "定义八：竞价设置为0.05"

            elif row['ACOS_7d'] > 0.5 and row['total_sales14d_3d'] == 0 and row['total_cost_3d'] > 3:
                new_keywordBid = 0.05
                bid_adjust = -1
                action_reason = "定义九：竞价设置为0.05"

            elif 0.24 < row['ACOS_7d'] < 0.36 and row['total_sales14d_3d'] == 0:
            ...
            elif row['total_clicks_7d'] > 13 and row['total_cost_7d'] > 8 and row['total_sales14d_7d'] == 0 and 0.24 < row[
                'ACOS_30d'] < 0.5:
                new_keywordBid = 0.05
                bid_adjust = -1
        ...
        # 循环遍历每个关键词
        for index, row in data.iterrows():
            new_keywordBid = row['bid']
            action_reason = ""
            bid_adjust = ""

            if row['ACOS_7d'] > 0.27 and row['ACOS_30d'] > 0.27 and row['ACOS_3d'] > 0.27:
                new_keywordBid = max(0.05, row['bid'] - 0.02) if row['bid'] >= 0.05 else \
                row['bid']
                bid_adjust = -0.02
                action_reason = "定义一"
    ...
    for index, row in output_df.iterrows():
        api.create_automatic_targeting_info(country, brand, '日常优化', '自动_劣质', row['keyword'], row['keywordId'],
